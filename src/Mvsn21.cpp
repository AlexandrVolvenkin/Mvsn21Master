...
////    pxMeasurementChannelRemap = &m_axMeasurementChannelRemap[0];
//
...
//                    ((axChipsChannelsData[m_axMeasurementChannelRemap[i].uiChip].
//                      // Выберем канал по индексу входа модуля, карты сопоставления измерительных каналов со входами модуля.
//                      axMeasurementChannels[m_axMeasurementChannelRemap[i].uiChannel].
    ...
    for (int8_t i = 0; i < DISCRETE_INPUT_NUMBER; )
    {
                ...
                j += 1)
        {
            ...
            if (i < DISCRETE_INPUT_NUMBER)
            {
                ...
                {
                    uiState >>= 1;
                    // Массив с состояниями измерительных каналов чипов.
                    // Состояние измерительного канала занимает два бита.
...
void CMvsn21::MeasureFsm(void)
{
    ...
    case FSM_CONTINUOUS_MEASURE:
        if (CAdc::MeasureIsComlete())
        {
            ...
            m_aucRtuDiscreteInputsArray[(m_uiChannel * 2) + (MEASURE_CHANNEL_NUMBER * MEASURE_CHANNEL_STATE_BIT_NUMBER * 2)] = (uiState & 0x01);
            m_aucRtuDiscreteInputsArray[(m_uiChannel * 2) + (MEASURE_CHANNEL_NUMBER * MEASURE_CHANNEL_STATE_BIT_NUMBER * 2) + 1] = ((uiState >> 1) & 0x01);
